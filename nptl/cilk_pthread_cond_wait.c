...
cilk_pthread_cond_wait_common (pthread_cond_t *cond, pthread_mutex_t *mutex,
    clockid_t clockid,
    const struct timespec *abstime)
{
  ...
  err = cilk_pthread_mutex_unlock_usercnt (mutex, 0);
  ...
  err = cilk_pthread_mutex_cond_lock (mutex);
  /* XXX Abort on errors that are disallowed by POSIX?  */
  return (err != 0) ? err : result;
}


/* See cilk_pthread_cond_wait_common.  */
int
cilk_pthread_cond_wait (pthread_cond_t *cond, pthread_mutex_t *mutex)
{
  /* clockid is unused when abstime is NULL. */
  return cilk_pthread_cond_wait_common (cond, mutex, 0, NULL);
}

/* See cilk_pthread_cond_wait_common.  */
int
cilk_pthread_cond_timedwait (pthread_cond_t *cond, pthread_mutex_t *mutex,
    const struct timespec *abstime)
{
  ...
  return cilk_pthread_cond_wait_common (cond, mutex, clockid, abstime);
}
